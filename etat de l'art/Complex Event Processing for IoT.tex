...
\section{Low Latency architecture}
...
\begin{figure}[h]
	\includegraphics[scale=0.5]{assets/Cloud_Computing.png}
...
The Fog Computing places the computing complexity at the local network level. Typically the complexity is put between the cloud and the IoT sensors. The devices are sending their data to a Middleware and the CEP engine is processing the data locally and send the processed events to a service running in the Cloud or on a local server.
